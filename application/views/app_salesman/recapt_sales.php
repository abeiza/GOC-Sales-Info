...
function loadAllSales(){
	...
	$.ajax({
		url:"<?php echo base_url();?>index.php/apps/get_sales_all_month_salesman/",
		cache:false,
...
function loadDetailSales(){
	var id_data = '<?php echo $this->session->userdata('id');?>';
	$.ajax({
		url:"<?php echo base_url();?>index.php/apps/get_sales_detail_month_salesman/",
		cache:false,
		data:"id_data="+id_data,
		type: "POST",
		dataType: 'json',
...
function loadAllSalesSearch(){
	...
	var year = $("#year").val();
	$.ajax({
		url:"<?php echo base_url();?>index.php/apps/get_sales_all_month_salesman_search/",
		cache:false,
		data: {
				"id_data": id_data, 
				"month": month,
...
function loadDetailSalesSearch(){
	...
	var year = $("#year").val();
	$.ajax({
		url:"<?php echo base_url();?>index.php/apps/get_sales_detail_month_salesman_search/",
		cache:false,
		data: {
				"id_data": id_data, 
				"month": month,
...
$(function(){
	$(function(){
		var m = $("#month").val();
		var y = $("#year").val();
		$("#graph").attr("href","http://112.78.137.253/GSI/index.php/apps/graphics_salesman/"+y+"/"+m);
	});
	...
	$("#month").change(function(){
		...
		$("#period span").remove();
		$("#period").append("<span style='color:#46b7bf;font-size:12px;'>Tanggal Transaksi : "+ '01' + "-" + m + "-" + y + " s/d "+ d + "-" + m + "-" + y + "</span>");
	...
	<div class="header-dashboard" style="height:54px !important; min-height:54px !important;">
		<div style="width:100%;position:absolute;z-index:99999;">
			<div class="content">
				...
				<div style="color:#fff;font-family:'Quicksand-Bold' !important;padding-top:17px;text-align:center;width:90%;float:left;">Rekapitulasi Penjualan</div>
			...
			<div style="width:100%;">
				<div class="box-flat">
					<div id="period" style="padding:5%;padding-bottom:10px;">
						...
						<span style="color:#46b7bf;font-size:12px;">Tanggal Transaksi : <?php echo date('01-m-Y');?> s/d <?php echo date('d-m-Y');?></span>
					</div>
				</div>
				<div class="box-flat">
					...
					<label style="float:Left;width:20%;padding:15px 0;">Bulan : </label>
						...
						<button id="search-btn" style="border:none;background-color: #ff8f00;color: #fff !important;float: Left;text-align: center;border-radius: 3px;width: 19% !important;padding: 10px;">Filter</button>
					</div>
				</div>
